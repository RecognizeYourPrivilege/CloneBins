...
| File | Distro |
| --- | --- |
| `CloneBins-0.1.2-ubuntu-amd64.deb` | Ubuntu / Debian (`sudo apt install ./…deb`) |
| `CloneBins-0.1.2-linux-x64.AppImage` | Generic glibc (`chmod +x` then run) |
| `CloneBins-0.1.2-archlinux-x86_64.pkg.tar.zst` | Arch (`sudo pacman -U`) |
| `CloneBins-0.1.2-windows-x64-setup.exe` | Windows 10/11 NSIS |
| `CloneBins-0.1.2-windows-x64.zip` | Windows portable zip |
...
apps/desktop/scripts/package-linux.sh deb \
  ...
  --out dist/release/CloneBins-0.1.2-ubuntu-amd64.deb
```
...
Not required for the unsigned v0.1.2 DMGs. Add these GitHub Actions secrets
...
DMGs and attaches them to the tag (`workflow_dispatch` with `tag=v0.1.2`
