...
This is a ReID-style stand-in: HSV color histogram + spatial color grid.
It groups images that share palette / clothing / backdrop — useful for
AI-generated character sets, and as the body half of ``face+body``.
...
class AppearanceEmbedder:
    ...
    def embed_vector(self, image_bgr: np.ndarray) -> np.ndarray:
        ...
        spatial = np.concatenate(cells).astype(np.float32)
        return l2_normalize(np.concatenate([hist, spatial]))
